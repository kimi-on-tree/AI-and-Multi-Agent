    ...
    public class CarAIControl : MonoBehaviour
    {
        ...
        private void FixedUpdate()
        {
            ...
            else
            {
                ...
                if (m_Rigidbody.linearVelocity.magnitude > m_CarController.MaxSpeed * 0.1f) fwd = m_Rigidbody.linearVelocity;

                var desiredSpeed = m_CarController.MaxSpeed;
                ...
                switch (m_BrakeCondition)
                {
                    case BrakeCondition.TargetDirectionDifference:
                    {
                        ...
                        var approachingCornerAngle = Vector3.Angle(m_Target.forward, fwd);

                        // also consider the current amount we're turning, multiplied up and then compared in the same way as an upcoming corner angle
                        var spinningAngle = m_Rigidbody.angularVelocity.magnitude * m_CautiousAngularVelocityFactor;

                        // if it's different to our current angle, we need to be cautious (i.e. slow down) a certain amount
                        var cautiousnessRequired = Mathf.InverseLerp(0, m_CautiousMaxAngle,
                    ...
                    case BrakeCondition.TargetDistance:
                    {
                        ...
                        var distanceCautiousFactor = Mathf.InverseLerp(m_CautiousMaxDistance, 0, delta.magnitude);

                        // also consider the current amount we're turning, multiplied up and then compared in the same way as an upcoming corner angle
                        var spinningAngle = m_Rigidbody.angularVelocity.magnitude * m_CautiousAngularVelocityFactor;

                        // if it's different to our current angle, we need to be cautious (i.e. slow down) a certain amount
                        var cautiousnessRequired = Mathf.Max(
                ...
                var offsetTargetPos = m_Target.position;
                ...
                var accelBrakeSensitivity = desiredSpeed < m_CarController.CurrentSpeed
                    ? m_BrakeSensitivity
                    : m_AccelSensitivity;

                // decide the actual amount of accel/brake input to achieve desired speed.
                var accel = Mathf.Clamp((desiredSpeed - m_CarController.CurrentSpeed) * accelBrakeSensitivity, -1, 1);
                ...
                var localTarget = transform.InverseTransformPoint(offsetTargetPos);

                // work out the local angle towards the target
                var targetAngle = Mathf.Atan2(localTarget.x, localTarget.z) * Mathf.Rad2Deg;

                // get the amount of steering needed to aim the car towards the target
                var steer = Mathf.Clamp(targetAngle * m_SteerSensitivity, -1, 1) * Mathf.Sign(m_CarController.CurrentSpeed);

                // feed input to the car controller.
                m_CarController.Move(steer, accel, accel, 0f);

                // if appropriate, stop driving when we're close enough to the target.
                if (m_StopWhenTargetReached && localTarget.magnitude < m_ReachTargetThreshold) m_Driving = false;
            }
        }
        ...
        private void OnCollisionStay(Collision col)
        {
            ...
            if (col.rigidbody != null)
            {
                ...
                if (otherAI != null)
                {
                    ...
                    var otherCarAngle = Mathf.Atan2(otherCarLocalDelta.x, otherCarLocalDelta.z);
